...
- TaskCreate: 添加 `deadline: Optional[date]`（可选）
...
- `type` 参数作为 CLI 侧任务标签写入 `description` 前缀
...
- [x] 数据库添加 `deadline` 字段
- [x] 后端 API 支持 deadline 参数
- [x] 前端新建任务表单包含截止日期选择
- [x] `ctx tasks new` 命令可用
- [x] 支持必填参数：title, deadline
- [x] 支持可选参数：type, priority, description
