...
| 子模块 | 文件 | 职责 |
|--------|------|------|
| 本地启动 | `scripts/start.sh` | 启动后端与前端并写入 PID |
| 本地停止 | `scripts/stop.sh` | 按 PID/进程名停止服务 |
...
| 后端环境模板 | `cortex-backend/.env.example`（若存在）或由 `start.sh` 兜底复制根目录 `.env.example` |
...
| 依赖安装机制 | `Dockerfile.backend` 按 `cortex-backend/pyproject.toml` 安装后端与 CLI 运行依赖，运行时代理支持必须在该依赖源声明 |
...
```bash
# 检查启动脚本读取与复制逻辑
rg -n "\\.env|\\.env\\.example|export \\$\\(grep" scripts/start.sh
