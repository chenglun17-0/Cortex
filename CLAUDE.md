...
- 所有任务遵循 "Ask → Code"
- **强制使用 ctx**：开始实施任何任务前，**必须**先执行 `ctx tasks start <任务ID>` 来启动任务，确保分支正确创建和绑定。**禁止手动创建分支**。如果 ctx 命令报错，必须先解决报错再重试 ctx 命令。
- 粒度：目标耗时 ≤ 1 小时；超出则先拆分
...
**⚠️ 重要规则**：
- **禁止手动创建分支**：所有分支必须通过 `ctx tasks start` 自动创建
- **禁止手动切换分支**：所有分支切换必须通过 ctx 命令完成
- **如果 ctx 报错**：必须先解决报错问题，再重试 ctx 命令
